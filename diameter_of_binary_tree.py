...
class Solution:
    def diameterOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        ...
        if not root:
            return 0
        return max(self.diameterOfBinaryTree(root.right), self.diameterOfBinaryTree(root.left), get_height(root.left) + get_height(root.right))
...
class Solution:
    def diameterOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        ...
        if not root:
            return 0
        return max( depth(root.right) + depth(root.left), self.diameterOfBinaryTree(root.left), self.diameterOfBinaryTree(root.right))